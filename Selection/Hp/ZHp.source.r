...
#print("Manhattan ploting Chromosomewise")
  ...
  pdf(paste("ZHp.", name.of.trait,".Chromosomewise.pdf" ,sep = ""), width = 10)
...
#  	y.lim <- ceiling(max(subset[,3]))  #set upper for each chr
    ...
    ylab = "Z(Hp)", main = paste("Chromosome",chm.to.analyze[i],sep=" "),cex.lab=1 , pch=20, xaxs="i", yaxs="i" )
...
s=size-wd/ratio/2
#y.lim <- ceiling(max(GI.MP[,3]))
y.lim <- floor(min(GI.MP[,3]))
#y.lim= 0.75
  pdf(paste("ZHp.", name.of.trait,".Genomewise.pdf" ,sep = ""), width = 15, height=3.5)
...
plot(y~x,ylab=expression(italic(paste("Z(H"[P],")",sep = ""))),  ylim=c(0,y.lim), xaxs="i", yaxs="i" ,type = "p", lwd=1.50, pch=20, cex.axis=0.5, cex.lab=1.0 ,col=mycols[z], axes=FALSE,  lty=1, cex=0.35, cex.main=2.0, xlab="Chromosome")
