...
def LoadPi(File,SNPNum):
    ...
    return Dict
@click.command()
@click.option('-1','--input1',help='Input 1 file [ref]',type=click.File('r'),required=True)
@click.option('-2','--input2',help='Input 2 file [target]',type=click.File('r'),required=True)
...
def main(input1,input2,snp,output):
    ...
    for line in input2:
        ...
        if index in RefPi:
            Pos = '\t'.join(line[:3])
            PiRatio = math.log(RefPi[index]/float(line[4]))
            output.write(f'{Pos}\t{PiRatio}\n')
