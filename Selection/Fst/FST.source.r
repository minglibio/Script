...
#print("Manhattan ploting Chromosomewise")
  ...
  pdf(paste("Fst.", name.of.trait,".Chromosomewise.pdf" ,sep = ""), width = 10)
  ...
  for(i in 1:numCHR)
  {
    ...
    ylab = "Fst", main = paste("Chromosome",chm.to.analyze[i],sep=" "),cex.lab=1 , pch=20, xaxs="i", yaxs="i" )
...
y.lim <- ceiling(max(GI.MP[,3]))
#y.lim= 0.75
  pdf(paste("Fst.", name.of.trait,".Genomewise.pdf" ,sep = ""), width = 15, height=3.5)
...
plot(y~x,ylab=expression(italic(paste("F"[ST],,sep = ""))),  ylim=c(0.05,y.lim), xaxs="i", yaxs="i" ,type = "p", lwd=1.50, pch=20, cex.axis=0.5, cex.lab=1.0 ,col=mycols[z], axes=FALSE,  lty=1, cex=0.35, cex.main=2.0, xlab="Chromosome")
...
#abline(h = Threshold, col="dimgray", lty=2, lwd=0.5)
abline(h=0.211886,col="dimgray",lty=2, lwd=1.0)  #MalVSInd
#abline(h=0.158785,col="dimgray",lty=2, lwd=1.0)  #MalVSInd
#abline(h=0.398446,col="dimgray",lty=2, lwd=1.0)  #IndVSPek
#abline(h=0.193225,col="dimgray",lty=2, lwd=1.0)  #IndVSPek
#abline(h=0.383862,col="dimgray",lty=2, lwd=1.0)  #MalVSPek
#abline(h=0.156592,col="dimgray",lty=2, lwd=1.0)  #MalVSPek
