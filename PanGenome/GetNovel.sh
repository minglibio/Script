...
bedtools subtract -a ${Prefix}.bed -b ${Prefix}.paf.bed > ${Prefix}.unmap
###### Remove gap of unmap region
bedtools subtract -a ${Prefix}.unmap -b ${Prefix}.N.bed > ${Prefix}.unmap.rmN.tmp
...
bedtools subtract -a ${Prefix}.unmap.rmN -b ${Prefix}.unmap.rmN.blastout.bed > ${Prefix}.unmap.rmN.rmBlast
###### Merge unmap region and remove region which length less than 500 bp
bedtools merge -d 200 -i ${Prefix}.unmap.rmN.rmBlast |awk '$3-$2>=500' > ${Prefix}.unmap.rmN.rmBlast.merge
###### Get seqence of unmap region
bedtools getfasta -fi ${Assembly} -bed ${Prefix}.unmap.rmN.rmBlast.merge -fo ${Prefix}.unmap.rmN.rmBlast.merge.fa
