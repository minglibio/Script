...
#Picard = 'java -Xmx220g -Djava.io.tmpdir=/stor9000/apps/users/NWSUAF/2015050469/tmp -jar /stor9000/apps/users/NWSUAF/2015050469/software/picard-2.18.7.jar'
...
def MarkDuplicates(Picard,Path,Name):
    return f"{Picard} MarkDuplicates INPUT={Path}/{Name}.sort.bam OUTPUT={Path}/{Name}.sort.dedup.bam METRICS_FILE={Path}/{Name}_dedup REMOVE_DUPLICATES=false CREATE_INDEX=true ASSUME_SORTED=true VALIDATION_STRINGENCY=LENIENT MAX_FILE_HANDLES=2000\n"
...
def RemoveList(Path,BamList,Name):
    Command = ''
    if len(BamList) > 1:
        for Bam in BamList: Command += f'rm {Path}/{Bam}.sort.bam\n'
...
def main(path,ref,output,queue,corenum,memory):
    ...
    if len(LeftFile) == 1:
        ...
        OUTPUT.write(MarkDuplicates(Picard,output,DirName))
        OUTPUT.write(f"echo {DirName} remove dup finished!\n")
    else:
