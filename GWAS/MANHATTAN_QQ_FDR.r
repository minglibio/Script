...
y.lim <- as.integer(ceiling(max(GI.MP[,3])))
#y.lim = 10
  pdf(paste("GWAS.FDR.", name.of.trait,".Manhattan-QQ Plot.pdf" ,sep = ""), width = 18,height=4.5)
...
plot(y~x,ylab=expression(-log[10](italic(FDR))) , ylim=c(0,y.lim), xaxs="i", yaxs="i" ,
...
abline(h=-log10(0.05),col="dimgray",lty=2, lwd=1)
...
p_value_quantiles <- (1:length(P.values))/(length(P.values)+1)
    ...
    qqplot(log.Quantiles, log.P.values, xlim = c(0,max(log.Quantiles)), ylim = c(0,max(log.P.values)),pch=20, mgp=c(1.6,0.5,0),
           cex.axis=1, cex.lab=1, lty = 1, lwd = 1.2, col = "Blue" ,xlab =expression(Expected~~-log[10](italic(FDR))), tck=-0.015,
           ylab = expression(Observed~~-log[10](italic(FDR)))) 
