...
def Submit(queue,corenum,memory,Output,ShellName,Command):
    os.system(f'jsub -R "rusage[res=1]span[hosts=1]" \
                     ...
                     -n {corenum} \
                     -M {memory*1000000} \
                     -o {Output}/IMM/shell/{ShellName}.o \
                     -e {Output}/IMM/shell/{ShellName}.e \
                     -J {ShellName} \
                     "{Command}"')
@click.command()
...
def main(chr,input,window,output,queue,corenum,memory):
    ...
    if not os.path.exists(f'{output}/IMM'): os.system(f'mkdir {output}/IMM')
    if not os.path.exists(f'{output}/IMM/shell'): os.system(f'mkdir {output}/IMM/shell')
    for line in chr :
        ...
        for i in range(1,Num + 1):
            if int(line[1]) - End <= window * 0.5:
                ...
                Command = f'python ~/software/IMCoalHMM-master/scripts/initial-migration-model.py --logfile {output}/IMM/{Region}.IMM.log -o {output}/IMM/{Region}.IMM.result {input}/{Region}'
